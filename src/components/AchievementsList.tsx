...
export const AchievementsList: React.FC<AchievementsListProps> = ({
  ...
  // Get unique categories (filter out null/undefined and dedupe)
  const categories = [
    'all',
    ...(Array.from(
      new Set(achievements.map((a) => a.category).filter((c): c is string => c !== null))
    ) as string[]),
  ]
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between">
            ...
            <div className="mt-4 md:mt-0">
              <div className="flex items-center">
                ...
                <span className="ml-3 text-lg font-medium text-gray-900">{stats.percentage}%</span>
              </div>
            </div>
          </div>
        </div>
