  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white shadow rounded-lg p-6 mb-6">
        ...
        {book.author && <p className="text-lg text-gray-600 mb-4">by {book.author}</p>}
      ...
      <div className="bg-white shadow rounded-lg p-8 mb-6">
        <h2 className="text-2xl font-bold text-gray-900 mb-6">{currentChapter.title}</h2>
...
const renderChapterContent = (chapter: ChapterContent, showFurigana: boolean) => {
  if (!chapter.furigana || !showFurigana) {
    ...
    // If content is an object, render it as JSON (temporary solution)
    return <p className="leading-relaxed whitespace-pre-line">{JSON.stringify(chapter.content)}</p>
  }

  // This is a simplified implementation
  // In a real application, you would want to properly parse and render furigana
  if (chapter.furigana && Array.isArray(chapter.furigana)) {
    return (
      <div className="leading-relaxed">
        {chapter.furigana.map((furigana, index) => (
          <ruby key={index} className="ruby-text">
            <span>{furigana.text}</span>
  ...
  return <div>Chapter content not available</div>
}
