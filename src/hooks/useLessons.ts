...
import type { LessonItem, LessonModule, UserLessonProgress } from '@/lib/database/types'
...
export const useLessons = (userId: string | undefined) => {
  ...
  const fetchModuleDetails = async (moduleId: number) => {
    try {
      const moduleData = await getLessonModuleById(moduleId)
      if (moduleData) {
  ...
  const updateProgress = async (moduleId: number, updates: Partial<UserLessonProgress>) => {
