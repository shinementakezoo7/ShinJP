...
export const getBooks = async (): Promise<Book[] | null> => {
  try {
    databaseLogger.debug('Fetching all published books')
    ...
    databaseLogger.info('Successfully fetched books', { count: data?.length || 0 })
    return data as Book[]
  } catch (error) {
...
export const getUserProgressForBook = async (
  userId: string,
  bookId: number
): Promise<BookReadingProgress | null> => {
  try {
