...
import type { StudySession, UserPerformance } from '../types'
...
export const getStudySessions = async (
  ...
  try {
    databaseLogger.debug('Fetching study sessions for user', { userId, limit })
    ...
    if (error) {
      databaseLogger.error('Error fetching study sessions', {
        userId,
        limit,
        error: error.message,
        code: error.code,
    ...
    databaseLogger.info('Successfully fetched study sessions', { userId, count: data?.length || 0 })
    return data as StudySession[]
  } catch (error) {
    databaseLogger.error(
      'Unexpected error fetching study sessions',
      { userId, limit },
      error as Error
    )
    return null
...
export const getStudySessionsInRange = async (
  ...
  try {
    databaseLogger.debug('Fetching study sessions in range for user', {
      userId,
      startDate,
      endDate,
    })
    ...
    if (error) {
      databaseLogger.error('Error fetching study sessions in range', {
        userId,
        startDate,
        endDate,
        error: error.message,
    ...
    databaseLogger.info('Successfully fetched study sessions in range', {
      userId,
      startDate,
      endDate,
      count: data?.length || 0,
    ...
    databaseLogger.error(
      'Unexpected error fetching study sessions in range',
...
export const createStudySession = async (
  ...
  try {
    databaseLogger.debug('Creating new study session', { session })
    ...
    if (error) {
      databaseLogger.error('Error creating study session', {
        session,
        error: error.message,
        code: error.code,
      })
    ...
    databaseLogger.info('Successfully created study session', { id: data.id })
    return data as StudySession
  } catch (error) {
    databaseLogger.error('Unexpected error creating study session', { session }, error as Error)
    return null
  }
}
...
export const endStudySession = async (
  ...
  endTime: string,
  duration: number,
  activities: Record<string, unknown> | SessionActivity[]
): Promise<StudySession | null> => {
  try {
    databaseLogger.debug('Ending study session', { sessionId, endTime, duration })
    ...
    if (error) {
      databaseLogger.error('Error ending study session', {
        sessionId,
        error: error.message,
        code: error.code,
      })
    ...
    databaseLogger.info('Successfully ended study session', { sessionId })
    return data as StudySession
  } catch (error) {
    databaseLogger.error('Unexpected error ending study session', { sessionId }, error as Error)
    return null
  }
}
...
export const getUserLearningStats = async (userId: string) => {
  try {
    databaseLogger.debug('Fetching user learning stats', { userId })
    ...
    databaseLogger.info('Successfully fetched user learning stats', { userId, stats })
    return stats
  } catch (error) {
    databaseLogger.error('Error fetching user learning stats', { userId }, error as Error)
    return null
  }
}
...
export const getDailyStudyTime = async (userId: string, days: number = 30) => {
  try {
    databaseLogger.debug('Fetching daily study time', { userId, days })
    ...
    databaseLogger.info('Successfully fetched daily study time', {
      userId,
      days,
    ...
    databaseLogger.error('Error fetching daily study time', { userId, days }, error as Error)
    return null
  }
}
...
export const getPerformanceTrends = async (userId: string, limit: number = 50) => {
  try {
    databaseLogger.debug('Fetching performance trends', { userId, limit })
    ...
    if (error) {
      databaseLogger.error('Error fetching performance trends', {
        userId,
        limit,
        error: error.message,
        code: error.code,
    ...
    data.forEach((performance: UserPerformance) => {
    ...
    databaseLogger.info('Successfully fetched performance trends', {
      userId,
      limit,
      dataPoints: result.length,
    })
    ...
    databaseLogger.error('Error fetching performance trends', { userId, limit }, error as Error)
    return null
  }
}
...
export const getJLPTProgress = async (userId: string) => {
  try {
    databaseLogger.debug('Fetching JLPT progress', { userId })
    ...
    databaseLogger.info('Successfully fetched JLPT progress', { userId })
    return progress
  } catch (error) {
    databaseLogger.error('Error fetching JLPT progress', { userId }, error as Error)
    return null
  }
}
